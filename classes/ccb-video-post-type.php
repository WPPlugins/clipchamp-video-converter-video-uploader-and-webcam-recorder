<?php
if ( ! class_exists( 'CCB_Video_Post_Type' ) ) {
    /**
     * Creates a custom post type and associated taxonomies
     */
    class CCB_Video_Post_Type extends CCB_Module implements CCB_Custom_Post_Type {
        protected static $readable_properties  = array();
        protected static $writeable_properties = array();
        const POST_TYPE_NAME = 'Clipchamp Video';
        const POST_TYPE_SLUG = 'clipchamp-video';
        ...
        public static function markup_meta_boxes( $post, $box ) {
            $variables = array();
            switch ( $box['id'] ) {
                case 'ccb_video-url-box':
                    $variables['videoUrl'] = get_post_meta( $post->ID, 'ccb_video-url', true );
                    $view                         = 'ccb-video-post-type/metabox-video-url.php';
                    break;
                /*
        ...
        protected static function save_custom_fields( $post_id, $new_values ) {
            if ( isset( $new_values[ 'ccb_video-url' ] ) ) {
                if ( false ) { // some business logic check
                    update_post_meta( $post_id, 'ccb_video-url', $new_values[ 'ccb_video-url' ] );
                } else {
